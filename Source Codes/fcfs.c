...
int n,top=-1,m,rqc=0,buffc=0;
int clk=-1,b=0;
int arr[20][10],buffer[2][1000];
int rq[1000],ioex[1000],allex[1000], flag[1000], buffx[1000];
...
void fcfs()
{
	...
	while(clk<=m+1)
	{
		...
		if(pro.burst==0)
		{
			...
			{			
				...
				else if(flag[rq[top]]==1)
				{
					pro.no=pop();
					if(arr[pro.no][4]==0)
						pro.burst=arr[pro.no][2];
					else
						pro.burst=arr[pro.no][4];
					flag[pro.no]--;
					allex[clk]=pro.no;
				}
	...
	arr[1][5]=0; //for WT 0
	arr[1][6]=arr[1][7]-arr[1][1]; //for TAT 0
	for(i=2;i<=n;i++)
	{
		arr[i][6]=arr[i][7]-arr[i][1]; //for TAT i
		arr[i][5]=arr[i][6]-arr[i][2]-arr[i][3]-arr[i][4]; //for WT i
	}
	display();	
}
